...
if use_managed_deps:
    ...
    if USE_SERVERLESS:
        spark = (
            DatabricksSession.builder.withEnvironment(env).serverless(True).getOrCreate()
        )
        ...
        spark = (
            DatabricksSession.builder.withEnvironment(env)
...
customers_df = spark.range(0, N_CUSTOMERS, numPartitions=PARTITIONS).select(
    F.concat(F.lit("CUST-"), F.lpad(F.col("id").cast("string"), 5, "0")).alias(
        "customer_id"
    ),
    ...
    F.date_sub(F.lit(START_DATE.date()), (F.rand(SEED) * 730).cast("int")).alias(
        "created_at"
    ),
)
...
customers_df.select("customer_id", "tier").write.mode("overwrite").saveAsTable(
    customers_tmp_table
)
...
orders_df = spark.range(0, N_ORDERS, numPartitions=PARTITIONS).select(
    ...
    F.date_add(F.lit(START_DATE.date()), (F.rand(SEED) * 180).cast("int")).alias(
        "order_date"
    ),
)
...
customer_lookup_with_idx = customer_lookup.withColumn(
    "customer_idx",
    (F.row_number().over(Window.orderBy(F.monotonically_increasing_id())) - 1).cast(
        "int"
    ),
)

# Join to get customer_id and tier as foreign key
orders_with_fk = orders_df.join(
    customer_lookup_with_idx, on="customer_idx", how="left"
).drop("customer_idx")
