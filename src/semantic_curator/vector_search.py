...
class VectorSearchManager:
    ...
    def __init__(self, 
                 ...
                 usage_policy_id: str | None = None) -> None:
    ...
    def create_or_get_index(self, wait_for_ready: bool = True) -> Any:
        ...
        try:
            ...
            if "RESOURCE_ALREADY_EXISTS" in str(e):
                ...
                logger.info(f"Index already exists, retrying to retrieve it: {self.index_name}")
    ...
    def _wait_for_index_ready(self, max_wait_seconds: int = 600) -> None:
        ...
        while time.time() - start_time < max_wait_seconds:
            try:
                index = self.client.get_index(index_name=self.index_name)
                status = index.describe().get("status", {}).get("ready", False) if hasattr(index, 'describe') else None
            ...
            logger.info(f"Waiting for index to be ready... ({int(elapsed)}s/{max_wait_seconds}s)")
            time.sleep(poll_interval)
        
        logger.warning(f"Index did not reach READY state within {max_wait_seconds} seconds, proceeding anyway")
        ...
        for attempt in range(max_retries):
            try:
                ...
                if "not found" in error_msg.lower() or "404" in error_msg:
                    logger.warning(f"Index not found (attempt {attempt + 1}/{max_retries}): {error_msg}")
                    ...
                    if attempt < max_retries - 1:
                        ...
                        logger.error(f"Index still not found after {max_retries} attempts")
                        raise
                else:
